...
#import "oi-wiki.typ": antiflash-white, info-blue, warning-orange, horizontalrule, blockquote, details, authors
`
  ...
  function parse(node) {
    ...
    const makeImage = function (loc) {
      ...
      try {
        ...
        return '#image("images/{0}", alt: "{1}")'.format(path.basename(dest), node.alt || '')
      } catch (e) {
    ...
    switch (node.type) {
      ...
      case 'heading': {
        const depth = Math.min(options.depth + Math.max(node.depth, 1), 6)
        return '{0} {1}'.format('='.repeat(depth), util.all(node, parse).join(''))
      }
      case 'text': {
      ...
      case 'inlineCode': {
        return '`{0}`'.format(escape(util.forceLinebreak(node.value)))
      }
      case 'code': {
        return '```{0}\n{1}\n```\n'.format(
          (function (lang) {
      ...
      case 'list': {
        return node.ordered
          ? node.start === 1
            ? util.all(node, parse).map(s => '+ ' + s).join('\n') + '\n'
            : '#block[#set enum(start: {0})\n{1}]'.format(
              node.start,
              util.all(node, parse).map(s => '+ ' + s).join('\n')) + '\n'
          : util.all(node, parse).map(s => '- ' + s).join('\n') + '\n'
      }
      case 'listItem': {
        return util.nonParagraphBegin(util.all(node, parse).join(''))
      }
      case 'thematicBreak': { // 水平分割线
        return '#horizontalrule' // 在印刷物中使用水平分割线也许不是好的实践
      }
      case 'blockquote': {
        return '#blockquote[{0}]'.format(util.all(node, parse).join(''))
      }
      case 'break': {
        return '\n\n'
      }
...
{3}
hlinex(),
)`.format('1fr, '.repeat(node.align.length),
          node.align.map(s => s ? s : 'center').join(', '),
      ...
      case 'details': { // Pymdown details 语法块
        ...
        return '#details({0}, {1})[{2}]'.format(color, title, util.all(node, parse).join(''))
      }
      default: {
