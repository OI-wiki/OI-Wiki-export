  ...
  for (const id in commands) {
    ...
    if (p2 === ntext.length) {
      ...
      return (
        ntext.slice(0, pos) +
        escape(rep) +
        escapeTextCommand(commands, ntext.slice(p2))
      );
    }
  }
  return text;
...
export function toPrefix(text) {
  return text.replace(/[^a-zA-Z0-9]/gi, "").replace(/md$/, "");
};

// 在 URL 后面拼接相对路径（去除 ? 和 # 后面的参数）
export function joinRelative(dir, options) {
  if (dir.indexOf("?") !== -1) {
...
export function forceLinebreak(text) {
  ...
  return text
    .split("")
    .map((char) => (isCjk(char) ? char : zwsp + char + zwsp))
    .join("")
...
export function isCjk(char) {
  ...
  if (code >= 0x4e00 && code <= 0x9fff) {
    ...
    // CJK 统一表意文字扩展 A 区 (Ext A)
    return true;
  } else if (code >= 0x20000 && code <= 0x2a6df) {
    // Ext B
    return true;
  } else if (code >= 0x2a700 && code <= 0x2b81f) {
    // Ext C & D
    return true;
  } else if (code >= 0x2b820 && code <= 0x2ebef) {
    // Ext E & F
    return true;
  } else if (code >= 0x30000 && code <= 0x3134f) {
    // Ext G
    return true;
  } else if (code >= 0xf000 && code <= 0xfaff) {
    // CJK 兼容表意文字 (CJK Compatibility Ideographs)
    return true;
  } else if (code >= 0x2f800 && code <= 0x2fa1f) {
    // CJK 表意文字补充 (Supplement)
    return true;
  } else {
    return false;
  }
