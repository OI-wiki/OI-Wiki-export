...
function compiler (options) {
  ...
  async function parse (node) {
    ...
    const makeImage = async function (loc) {
      ...
      try {
        ...
        switch (ext) {
          ...
          case '.jpeg': {
            if (!await util.exists(dest)) {
              await fs.copyFile(uri, dest)
            }
            break
          }
          case '.svg': {
            if (!await util.exists(dest)) {
              await execFile('inkscape', [`--export-filename=${dest}`, uri])
            }
            break
          }
          default: {
            if (!await util.exists(dest)) {
              ...
              await execFile('convert', ['-background', 'white', '-flatten', ext === '.gif' ? uri + '[0]' : uri, dest])
            }
            break
          }
        }
